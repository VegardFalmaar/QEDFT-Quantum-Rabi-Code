...
from typing import Callable, Union, Iterable,List, Dict, Any, Optional
...
class Basis:
    ...
    def __init__(self, qn_key: Optional[str] = None, qn_range: Optional[Iterable[Union[int, float]]] = None):
        """
        ...
        if not qn_key is None and not qn_range is None:
            if not isinstance(qn_key, str): raise TypeError(f'The qn key must be a string, got {type(qn_key).__name__}.')
            self.part_num = 1
            self.qn_keys = {qn_key}
    ...
    def diag(self, qn_key: Optional[str] = None) -> 'Operator':
        """
    ...
    def hop(self, qn1: dict, qn2: dict|None = None) -> 'Operator': 
        """
...
class OperatorList: # list of operators
    ...
    def conj(self) -> 'OperatorList': # conjugate
        """
        Returns the complex conjugate of each operator in the list.

        Returns:
            OperatorList: A new OperatorList with the conjugate of each operator.
        """
        B = OperatorList(self.basis)
        for op in self.operators:
            B.append(op.conj())
    ...
    def tensor(self, A: 'Operator', tensor_basis: Basis = None) -> 'OperatorList': 
        """
